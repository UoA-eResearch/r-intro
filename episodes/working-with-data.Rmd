...
We'll start off with two of the most commonly used `dplyr` functions: `select()`, which selects certain columns of a data.frame, and `filter()`, which keep rows according to certain criteria.
...
We're using the original version of the pipe introduced to R via the tidyverse (`%>%`), but it's worth pointing out, that since R 4.1.0+ there is now a native pipe operator (`|>`). You may see the native pipe operator in other online resources or communities. RStudio provides an option to switch under Tools > Global options > Code > Use native pipe operator. There are some subtle differences between the two, which you can read more about [here](https://www.tidyverse.org/blog/2023/04/base-vs-magrittr-pipe/). 
