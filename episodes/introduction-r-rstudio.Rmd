...
- Be able to format questions to get help from the broader R community
...
It is good practice to organize your projects into self-contained folders right from the start, so we will start building that habit now. A well-organized project is easier to navigate, more reproducible, and easier to share with others. Your project should start with a top-level folder that contains everything necessary for the project, including data, scripts, and images, all organized into sub-folders.
...
5. Type out a name for the project, we recommend `r-intro`.
6. Put it in a convenient location using the "Create project as a subdirectory of:" section. For now, we recommend your `Desktop`. You can always move the project somewhere else later, because it will be self-contained.
7. Click **Create Project** and your new project will open.

We're creating a new project for the purposes of this workshop, but if you wanted to use the RStudio project feature with an existing folder of work, then you could select that folder and set up the r project there by selecting **Existing Directory** after selecting **New Project**.
...
One of the benefits to using RStudio Projects is that they automatically set the **working directory** to the folder you chose for the project. The working directory is the folder where R is working, so it views the location of all files (including data and scripts) as being relative to the working directory. You may come across scripts that include something like `setwd("/Users/YourUserName/MyCoolProject")`, which directly sets a working directory. This is usually much less portable, since that specific directory might not be found on someone else's computer (they probably don't have the same username as you). Using RStudio Projects means we don't have to deal with manually setting the working directory.
